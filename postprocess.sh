...
process () {
	...
	ll=($(cat to_exec.lst.iceshelves | awk '{print $5}' | sed 's/cfgfiles\///' | awk -F.ini '{print $1}'))
		...
		unzip -p ./${z} output/${f} | mawk 'BEGIN {data=0; mswind=-1; mssnow=-1; swe=-1; hs=-1; msmelt=-1; qs=-1; ql=-1; qr=-1; iswr=-1; oswr=-1; lwrnet=-1; a=-1;} {if(/longitude/) {lon=$NF}; if(data==1) {a=mktime(sprintf("%04d %02d %02d %02d %02d %02d 0", substr($1,1,4), substr($1,6,2), substr($1,9,2), substr($1,12,2), substr($1,15,2), substr($1,18,2))); print $1, a, strftime("%Y-%m-%dT%H:%M:%S", aloc), aloc, $mswind, $mssnow, $swe, ($hs/100.), $msmelt, $qs, $ql, $qr, $iswr-$oswr, $lwrnet}; if(/\[DATA\]/) {data=1}; if(/fields/) {for(i=3; i<=NF; i++) {if($i=="MS_Wind") {mswind=(i-2)}; if($i=="MS_Snow") {mssnow=(i-2)}; if($i=="HS_mod") {hs=(i-2)}; if($i=="SWE") {swe=(i-2)}; if($i=="MS_melt") {msmelt=(i-2)}; if($i=="Qs") {qs=(i-2)}; if($i=="Ql") {ql=(i-2)}; if($i=="Qr") {qr=(i-2)}; if($i=="LWR_net") {lwrnet=(i-2)}; if($i=="ISWR") {iswr=(i-2)}; if($i=="OSWR") {oswr=(i-2)}}}}' >> ${g}
